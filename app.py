...
def get_farmer_by_phone(phoneNumber):
    ...
    print(f"DEBUG: Looking for phone '{phoneNumber}'")
    
    try:
        records = sheet_farmers.get_all_records()
        print(f"DEBUG: Total farmers: {len(records)}")
        
        for idx, row in enumerate(records, start=2):
            sheet_phone = str(row.get('Phone (MSISDN)', '')).strip()
            if sheet_phone == phoneNumber:
                print(f"DEBUG: Farmer found: {row.get('Full Name', 'Unknown')}")
                return {
                    "row_num": idx,
        ...
        print(f"DEBUG: No farmer found for '{phoneNumber}'")
        return None
    except Exception as e:
...
def get_farmer_deliveries(farmer_id):
    ...
    print(f"DEBUG: Getting deliveries for farmer_id '{farmer_id}'")
    ...
    try:
        ...
        print(f"DEBUG: Found {len(deliveries)} deliveries")
        return deliveries
    except Exception as e:
        print(f"DEBUG: Error: {e}")
        return []
...
def ussd():
    ...
    print(f"DEBUG: farmer found = {farmer is not None}")
