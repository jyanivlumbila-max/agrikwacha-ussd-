...
def get_farmer_by_phone(phoneNumber):
    """Find farmer by Phone (MSISDN) field"""
    phoneNumber = phoneNumber.strip()
    print(f"Looking up phone: '{phoneNumber}'")
    
    filter_formula = f'{{Phone (MSISDN)}} = "{phoneNumber}"'
    result = call_airtable(f"{TABLE_FARMERS}?filterByFormula={filter_formula}")
    
    if result and result.get('records'):
        farmer = result['records'][0]
        print(f"Farmer found: {farmer['fields'].get('Full Name', 'Unknown')}")
        return farmer
    else:
...
def get_farmer_deliveries(farmer_id):
    """Get all deliveries for a farmer from Deliveries table"""
...
def get_pending_delivery_by_ref(delivery_ref, farmer_id):
    """Find a pending delivery by reference for a specific farmer"""
...
def confirm_delivery(delivery_ref, farmer_id):
    ...
    # First find the delivery record
...
def create_delivery(delivery_ref, farmer, product, depot_code, quantity_kg, produce_value):
    ...
    result = call_airtable(TABLE_DELIVERIES, "POST", data)
    print(f"Delivery created: {delivery_ref}")
    return delivery_ref

def get_all_depots():
    """Get all active depots"""
    result = call_airtable(f"{TABLE_DEPOTS}?filterByFormula={{Status}} = \"Active\"")
    if result and result.get('records'):
        return result['records']
    return []
...
def calculate_produce_value(product, quantity_kg):
    ...
    # Default prices per kg (you can adjust these or read from a Products table)
    prices = {
        "Maize": 5.00,
        "Soya Beans": 6.00,
        "Cattle": 25.00,
...
def ussd():
    ...
    if main_option == "1":
        ...
        # Level 2: Product selected, ask for quantity
        elif level == 2:
            product_map = {
        ...
        # Level 3: Quantity entered, show depot menu
        elif level == 3:
            try:
                ...
                if depots:
                    ...
                    response = "CON Select depot.\n"
                    response += "1. Zambeef Lusaka\n"
                    response += "2. AFGRI Mkushi\n"
                    response += "3. Mwila Mills Ndola\n"
                    response += "0. Back"
        ...
        # Level 4: Depot selected, show confirmation
        elif level == 4:
            depot_map = {
                "1": "Zambeef Lusaka",
                "2": "AFGRI Mkushi",
            ...
            quantity = sessions.get(sessionId, {}).get("quantity", 0)
            estimated_value = sessions.get(sessionId, {}).get("estimated_value", 0)
            ...
            response += f"Estimated value: ZMW {estimated_value}\n"
            response += "1. Confirm\n"
            response += "2. Cancel"
            return send_response(response)
    ...
    # ========== OPTION 2: My Payments ==========
        ...
        # Filter for paid/confirmed deliveries
    ...
    # ========== OPTION 3: Confirm Delivery ==========
        ...
        if level == 1:
            ...
            if user_input[2] == "1":
                ...
                if confirm_ref:
                    confirm_delivery(confirm_ref, farmer_id)
    ...
    # ========== OPTION 4: Help ==========
        ...
        response += "1. Register Delivery: Record your harvest\n"
        response += "2. My Payments: Check your payment history\n"
        response += "3. Confirm Delivery: Confirm a pending delivery\n"
        response += "Call support: 0977 123 456"
        return send_response(response)
...
def debug_phone():
    # This simulates what the simulator sends
    phone_from_simulator = "+260973355333"
    
    # Check what's in Airtable
    filter_formula = f'{{Phone (MSISDN)}} = "{phone_from_simulator}"'
    result = call_airtable(f"Farmers?filterByFormula={filter_formula}")
    
    # Also try without the +
    phone_without_plus = "260973355333"
    filter_formula2 = f'{{Phone (MSISDN)}} = "{phone_without_plus}"'
    result2 = call_airtable(f"Farmers?filterByFormula={filter_formula2}")
    
    # Also try with 0 instead of +260
    phone_with_zero = "0973355333"
    filter_formula3 = f'{{Phone (MSISDN)}} = "{phone_with_zero}"'
    result3 = call_airtable(f"Farmers?filterByFormula={filter_formula3}")
    
    return {
        "simulator_sends": phone_from_simulator,
        "with_plus_found": len(result.get('records', [])) if result else 0,
        "with_plus_data": result.get('records', []) if result else None,
        "without_plus_found": len(result2.get('records', [])) if result2 else 0,
        "with_zero_found": len(result3.get('records', [])) if result3 else 0,
        "all_farmers": get_all_farmers_phones()
    }

def get_all_farmers_phones():
    """Helper to see all phone numbers in your Farmers table"""
    result = call_airtable("Farmers?maxRecords=50")
    phones = []
    if result and result.get('records'):
        for record in result['records']:
            phone = record['fields'].get('Phone (MSISDN)', 'Not set')
            phones.append(phone)
    return phones
@app.route('/debug/fields', methods=['GET'])
def debug_fields():
    result = call_airtable("Farmers?maxRecords=1")
    if result and result.get('records'):
        fields = list(result['records'][0].get('fields', {}).keys())
        return {
            "available_fields": fields,
            "sample": result['records'][0].get('fields')
        }
...
def debug_env():
    ...
    return {
        ...
        "base_id_exists": bool(base_id),
        "base_id_start": base_id[:10] if base_id else "None",
        "api_key_length": len(api_key),
        "base_id_length": len(base_id)
    }
@app.route('/debug/airtable-test', methods=['GET'])
def debug_airtable_test():
    # Try to list all tables in your base
    url = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Farmers?maxRecords=5"
