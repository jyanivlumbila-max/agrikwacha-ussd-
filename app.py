...
def get_products():
    """Return list of available products (could also come from Airtable)"""
    return [
        {"code": "1", "name": "Maize"},
...
def ussd():
    ...
    if main_option == "1":
        ...
        # Level 1: Show products
        if level == 1:
            products = get_products()
            response = "CON Select product. "
            for p in products:
                response += f"{p['code']}. {p['name']}. "
            response += "0. Back"
            return send_response(response)
        ...
        # Level 4: Depot selected
            ...
            response = f"CON Confirm delivery. Product: {product}. Quantity: {quantity} kg. Depot: {depot_name}. Value: ZMW {estimated_value}. 1. Confirm. 2. Cancel"
            return send_response(response)
...
def send_response(message):
    """Send USSD response - single line only"""
