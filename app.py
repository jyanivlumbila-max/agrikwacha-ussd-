...
@app.route('/debug/check-deliveries', methods=['GET'])
def debug_check_deliveries():
    ...
    if not farmer:
        return {"error": "Farmer not found", "phone": phone}
    
    farmer_id = farmer['fields'].get('Farmer ID', '')
    farmer_name = farmer['fields'].get('Full Name', 'Unknown')
    ...
    if result and result.get('records'):
        for record in result['records']:
            fields = record.get('fields', {})
            deliveries.append({
                "ref": fields.get('Delivery Ref', ''),
                "date": fields.get('Date', ''),
                "product": fields.get('Product', ''),
                "value": fields.get('Produce Value ZMW', 0),
                "status": fields.get('Status', ''),
                "confirmed": fields.get('Farmer Confirmed?', '')
            })
...
def debug_status():
    ...
    farmer_id = farmer['fields'].get('Farmer', '')
    
    filter_formula = f'{{Farmer}} = "{farmer_id}"'
    result = call_airtable(f"{TABLE_DELIVERIES}?filterByFormula={filter_formula}")
...
@app.route('/debug/phone-debug', methods=['GET'])
def debug_phone_debug():
    # This simulates what the simulator sends
    test_phone = "+260973355333"
    
    # Get all farmers from Airtable
    result = call_airtable(f"{TABLE_FARMERS}?maxRecords=50")
    ...
    if result and result.get('records'):
        for record in result['records']:
            ...
            farmers.append({
                ...
                "farmer_id": fields.get('Farmer', ''),
                "phone_raw": repr(phone_value),  # shows hidden characters
                "phone_stripped": str(phone_value).strip() if phone_value else None
            })
    
    # Try to find the test phone
    found = False
    for f in farmers:
        if f['phone_stripped'] == test_phone:
            found = True
            break
    
    return {
        "test_phone": test_phone,
        "test_phone_repr": repr(test_phone),
        "farmers": farmers,
        "test_phone_found": found
    }
@app.route('/debug/confirm', methods=['GET'])
def debug_confirm():
    delivery_ref = request.args.get("ref", "")
    phone = request.args.get("phone", "").strip()  # ← Added .strip()
    
    if not delivery_ref or not phone:
        return {"error": "Provide ?ref=DLV-79990&phone=+260973355333"}
    
    print(f"Looking for farmer with phone: '{phone}'")
    
    # Find farmer
    farmer = get_farmer_by_phone(phone)
    if not farmer:
        return {"error": f"Farmer not found for phone: '{phone}'"}
    
    farmer_id = farmer['fields'].get('Farmer', '')
    farmer_name = farmer['fields'].get('Full Name', '')
    
    # Find the delivery
    filter_formula = f'AND({{Delivery Ref}} = "{delivery_ref}", {{Farmer}} = "{farmer_id}")'
    result = call_airtable(f"{TABLE_DELIVERIES}?filterByFormula={filter_formula}")
    ...
    if result and result.get('records'):
        for record in result['records']:
            ...
            deliveries.append({
                "ref": fields.get('Delivery Ref', ''),
                "status": fields.get('Status', ''),
                "farmer_confirmed": fields.get('Farmer Confirmed?', ''),
                "product": fields.get('Product', ''),
                "value": fields.get('Produce Value ZMW', 0)
            })
    ...
    return {
        "farmer_name": farmer_name,
        "farmer_id": farmer_id,
        "search_ref": delivery_ref,
        "deliveries_found": deliveries
    }
@app.route('/debug/all-deliveries', methods=['GET'])
def debug_all_deliveries():
    # Get all deliveries from Airtable
    result = call_airtable(f"{TABLE_DELIVERIES}?maxRecords=50")
    ...
    if result and result.get('records'):
        for record in result['records']:
            ...
            deliveries.append({
                "ref": fields.get('Delivery Ref', ''),
                "farmer_id": fields.get('Farmer ID', ''),
                "status": fields.get('Status', ''),
                "product": fields.get('Product', '')
            })
