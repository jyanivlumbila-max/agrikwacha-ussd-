...
def debug_all():
    ...
    results["env_vars"] = {
        ...
        "API_KEY_first_10": AIRTABLE_API_KEY[:10] if AIRTABLE_API_KEY else "None",
        "BASE_ID_first_10": AIRTABLE_BASE_ID[:10] if AIRTABLE_BASE_ID else "None"
    }
    ...
    if AIRTABLE_API_KEY and AIRTABLE_BASE_ID:
        ...
        try:
            ...
            if response.status_code == 200:
                results["airtable_connection"]["first_record"] = response.json().get("records", [])[:1]
        except Exception as e:
            results["airtable_connection"] = {"status": "error", "message": str(e)}
    else:
        results["airtable_connection"] = {"status": "error", "message": "Missing credentials"}
    
    # Test 3: List all farmers
    if AIRTABLE_API_KEY and AIRTABLE_BASE_ID:
        url = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Farmers"
        headers = {"Authorization": f"Bearer {AIRTABLE_API_KEY}"}
        try:
            response = requests.get(url, headers=headers)
            if response.status_code == 200:
                ...
                for record in records[:10]:
                    ...
                    farmers_list.append({
                        ...
                        "phone": fields.get("Phone (MSISDN)", "Unknown")
                    })
                results["farmers"] = {
                    "count": len(records),
                    "sample": farmers_list
                ...
                results["farmers"] = {"error": f"Status {response.status_code}", "response": response.text[:200]}
        except Exception as e:
            results["farmers"] = {"error": str(e)}
    
    return results

@app.route('/debug/farmer', methods=['GET'])
def debug_farmer():
    """Test finding a specific farmer by phone"""
    ...
    # Test 1: Exact match with +260
    filter1 = f'{{Phone (MSISDN)}} = "{phone}"'
    url1 = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Farmers?filterByFormula={filter1}"
    response1 = requests.get(url1, headers=headers)
    
    results["tests"].append({
        "description": "Exact match with +260",
        "filter": filter1,
        "status": response1.status_code,
        "records_found": len(response1.json().get("records", [])) if response1.status_code == 200 else 0
    })
    
    # Test 2: Try without +260
    phone2 = phone.replace("+260", "")
    filter2 = f'{{Phone (MSISDN)}} = "{phone2}"'
    url2 = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Farmers?filterByFormula={filter2}"
    ...
    results["tests"].append({
        ...
        "records_found": len(response2.json().get("records", [])) if response2.status_code == 200 else 0
    })
    
    # Test 3: Try with 0 prefix
    phone3 = "0" + phone.replace("+260", "")
    filter3 = f'{{Phone (MSISDN)}} = "{phone3}"'
    url3 = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Farmers?filterByFormula={filter3}"
    response3 = requests.get(url3, headers=headers)
    
    results["tests"].append({
        "description": "With 0 prefix",
        "filter": filter3,
        "status": response3.status_code,
        "records_found": len(response3.json().get("records", [])) if response3.status_code == 200 else 0
    })
...
def debug_airtable():
    """Test raw Airtable API call"""
    phone = request.args.get("phone", "+260973355333")
    filter_formula = f'{{Phone (MSISDN)}} = "{phone}"'
    url = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Farmers?filterByFormula={filter_formula}"
    headers = {"Authorization": f"Bearer {AIRTABLE_API_KEY}"}
    ...
    return {
        ...
        "records_count": len(response.json().get("records", [])) if response.status_code == 200 else 0
    }

@app.route('/debug/test-ussd', methods=['GET'])
def debug_test_ussd():
    """Simulate a USSD request and show what would happen"""
    phone = request.args.get("phone", "+260973355333")
    ...
    return {
        ...
        "farmer_data": {
            "id": farmer.get("id") if farmer else None,
            "name": farmer.get("fields", {}).get("Full Name") if farmer else None,
            "phone": farmer.get("fields", {}).get("Phone (MSISDN)") if farmer else None
        } if farmer else None,
        "would_show": "Main menu" if farmer else "You are not registered message"
    }
